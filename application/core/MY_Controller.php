...
class MY_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // $host = 'losin_academy.losinsms.com';
        $parts = explode('.', $host);
        $subdomain = null;
        if (count($parts) >= 3) {
            if ($parts[0] !== $host) {
        ...
        if ($subdomain) {
            $branch_config = $this->db->get_where('front_cms_setting', array('url_alias' => $subdomain))->row_array();

            if ($branch_config) {
                $get_config['institute_name'] = $branch_config['application_title'];
                $get_config['logo_file'] = $branch_config['logo'];
                $get_config['footer_text'] = $branch_config['footer_about_text'];
            }
        }
