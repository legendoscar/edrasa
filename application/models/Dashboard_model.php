...
class Dashboard_model extends CI_Model
{
    ...
    public function getMonthlyBookIssued($id = '')
    {
        ...
        $this->db->where(
            "start_date BETWEEN DATE_SUB(CURDATE() ,INTERVAL 1 MONTH) AND CURDATE() AND status = '2' AND role_id = '7' AND user_id = " .
                $this->db->escape($id)
        );
    ...
    public function getStaffCounter($role = '', $branchID = '')
    {
        ...
        $this->db->join(
            'login_credential',
            'login_credential.user_id = staff.id',
            'inner'
        );
        ...
        if (!empty($role)) {
            ...
            $this->db->where_not_in('login_credential.role', [1, 3, 6, 7]);
        }
        if (!empty($branchID)) {
    ...
    public function getMonthlyPayment($id = '')
    {
        ...
        $this->db->join(
            'fee_payment_history as h',
            'h.allocation_id = fa.id',
            'left'
        );
        $this->db->where(
            'h.date BETWEEN DATE_SUB(CURDATE(),INTERVAL 1 MONTH) AND CURDATE() AND fa.student_id = ' .
                $this->db->escape($id) .
                ' AND fa.session_id = ' .
    ...
    public function getStudentAttendance($studentID = '')
    {
        ...
        for ($month = 1; $month <= 12; $month++):
            $total_present[] = $this->db
                ->query(
                    'SELECT id FROM student_attendance WHERE MONTH(date) = ' .
                        $this->db->escape($month) .
                        " AND YEAR(date) = YEAR(CURDATE()) AND status = 'P' AND student_id = " .
            ...
            $total_absent[] = $this->db
                ->query(
                    'SELECT id FROM student_attendance WHERE MONTH(date) = ' .
                        $this->db->escape($month) .
                        " AND YEAR(date) = YEAR(CURDATE()) AND status = 'A' AND student_id = " .
                        $this->db->escape($studentID)
                )
                ->num_rows();
            $total_late[] = $this->db
                ->query(
                    'SELECT id FROM student_attendance WHERE MONTH(date) = ' .
                        $this->db->escape($month) .
                        " AND YEAR(date) = YEAR(CURDATE()) AND status = 'L' AND student_id = " .
                        $this->db->escape($studentID)
                )
                ->num_rows();
        endfor;
    ...
    public function get_monthly_attachments($id = '')
    {
        ...
        $this->db->where(
            'date BETWEEN DATE_SUB(CURDATE() ,INTERVAL 1 MONTH) AND CURDATE() AND (class_id = ' .
                $this->db->escape($classID) .
                " OR class_id = 'unfiltered') AND branch_id = " .
    ...
    public function getWeekendAttendance($branchID = '')
    {
        ...
        foreach ($period as $day) {
            ...
            $this->db->where(
                'date = "' .
                    $day->format('Y-m-d') .
                    '" AND (status = "P" OR status = "L")'
            );
            $student_att[]['y'] = $this->db
    ...
    {
        $query =
            'SELECT IFNULL(SUM(dr),0) as dr, IFNULL(SUM(cr),0) as cr FROM transactions WHERE month(DATE) = MONTH(now()) AND year(DATE) = YEAR(now())';
        if (!empty($branchID)) {
            $query .= ' AND branch_id = ' . $this->db->escape($branchID);
        }
        $r = $this->db->query($query)->row_array();
        return [
            ['name' => translate('expense'), 'value' => $r['dr']],
            ['name' => translate('income'), 'value' => $r['cr']],
        ];
    }
    ...
    {
        $this->db->select(
            'IFNULL(COUNT(e.student_id), 0) as total_student, c.name as class_name'
        );
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($students as $row) {
                $data[] = [
                    'value' => floatval($row->total_student),
                    'name' => $row->class_name,
                ];
            }
        } else {
    ...
    public function getMonthlyAdmission($branchID = '')
    {
        ...
        $this->db->where(
            's.admission_date BETWEEN DATE_SUB(CURDATE() ,INTERVAL 1 MONTH) AND CURDATE()'
        );
        if (!empty($branchID)) {
            $this->db->where('e.branch_id', $branchID);
        }
    ...
    public function getVoucher($branchID = '')
    {
        ...
        $this->db->where(
            'date BETWEEN DATE_SUB(CURDATE() ,INTERVAL 1 MONTH) AND CURDATE()'
        );
